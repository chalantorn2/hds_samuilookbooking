...
// Format ตารางแบบ by Supplier - แนวเดียวกับ CustomerFilterTable
const SupplierFilterTable = ({
  reportData,
  startDate,
  endDate,
  selectedSupplier,
  ...
  // Group bookings by supplier_code
  const groupedBySupplier = allBookings.reduce((acc, booking) => {
    const key = booking.supplier_code || "UNKNOWN";
    if (!acc[key]) {
      acc[key] = {
        supplier_code: key,
  ...
  // Sort supplier groups by supplier_code
  ...
  const isAllSuppliers = !selectedSupplier;

  const handleExportExcel = () => {
    const exportData = [];

    // Add header
    exportData.push(["REPORT BY SUPPLIER"]);
    if (!isAllSuppliers) {
      exportData.push([`For Supplier: ${selectedSupplier}`]);
    }
    exportData.push([
    ...
    // Table header (same as ALL Invoice but without SUP, show CUST instead)
    const tableHeader = [
      "No",
      "Date",
      "Ref No.",
    ...
    const filename = isAllSuppliers
      ? `Supplier_Report_ALL_${startDate}_${endDate}.xlsx`
      : `Supplier_Report_${selectedSupplier}_${startDate}_${endDate}.xlsx`;
    XLSX.writeFile(wb, filename);
  };

  // Render table for a single supplier group
  const renderSupplierTable = (bookings, subTotal) => (
              ...
              <div key={groupIndex} className="mb-8">
                ...
                <div className="bg-blue-800 text-white px-4 py-2 rounded-t-md">
                  <h3 className="font-semibold">
                    {group.supplier_code} - {group.supplier_name}
                  </h3>
                </div>

                {/* Supplier Table */}
                {renderSupplierTable(sortedBookings, subTotal)}
              </div>
            );
          })}
...
export default SupplierFilterTable;
